...
# S(2) Feature Selection Budget (e.g., 37.5% of the Haystack).
BUDGET_PCT <- .375
M_PREDICTORS <- floor(M_POOL_SIZE * BUDGET_PCT)

# "Dynamic Range" of the S(2).
PARAMETRIC_MEAN_MIN <- .25
PARAMETRIC_MEAN_MAX <- (1 - PARAMETRIC_MEAN_MIN)
...
find_spectral_elbow <- function(eig_vals, noise_floor) {
  ...
  # --- ELBOW DETECTION SENSITIVITY CONFIGURATION ---
...
run_latent_sparsity_analysis <- function(n_iterations = 20) {
  cat("--- Running Part 1a: S(1) Latent Complexity Simulation ---\n")
  results_list <- list()
  ...
  cat("S(1) Latent Complexity Simulation Complete.\n")
...
plot_effective_configurations <- function(results_df, n_iterations) {
  ...
  p <- ggplot(plot_data, aes(x = Scenario_Label, y = Mean, fill = Metric)) +
    ...
    labs(title = "S(1) Latent Complexity Simulation",
...
run_spectral_analysis_and_plot <- function(k_eff_from_s1, n_iterations = 20, calc_eigenvals_raw_func) {
  ...
  avg_est_k_obs <- mean(est_k_obs_values)
  ...
  p <- ggplot(agg_plot_data_subset, aes(x = Component, y = Mean_Eigenvalue, color = DataSource, linetype = DataSource)) +
    ...
    # Observed Estimate Line and Label
    ...
    annotate("text", x = avg_est_k_obs * 1.2, y = y_pos_est_obs, 
             label = sprintf("Effective Signal Rank (r): %.2f", avg_est_k_obs), 
...
run_strategy_spectral_comparison <- function() {
  ...
  cat("    Benchmarking all strategies using Dynamic Haystack Elbows...\n")
  ...
  for (i in 1:n_iters) {
    ...
    # Grab dynamic elbow for this specific iteration
    dynamic_k_target <- max(1, run_obj$k_eff_empirical[["Observed"]])
    
    cat(sprintf("\r    Reconstructing Dataset for Iteration %d (Seed %d, k=%d)...", i, iter_seed, dynamic_k_target))
    ...
    # 3. CALCULATE SPECTRA FOR THIS ITERATION (Using dynamic_k_target)
    ...
    for(strat in strategies) {
      ...
      if (!is.null(selected)) {
        ...
        # Calculate SNR for this specific run using dynamic_k_target
        val_sig <- ev[dynamic_k_target]
        val_noise <- if((dynamic_k_target + 1) <= length(ev)) ev[dynamic_k_target + 1] else 1e-9
        spec_snr <- val_sig / val_noise
        
        energy_sig <- sum(ev[1:dynamic_k_target])
        energy_noise <- sum(ev[(dynamic_k_target+1):length(ev)])
  ...
  # Average Metrics AND Average Elbow
  agg_stats <- full_data[, .(Spectral_SNR = mean(unique(Spectral_SNR)), 
                             Overall_SNR = mean(unique(Overall_SNR)),
                             Avg_K = mean(unique(Dynamic_K))), by = .(Strategy)]
  
  # Get the global average K across all strategies/iterations for plotting the line
  mean_k_plot <- mean(agg_stats$Avg_K)
  ...
  p <- ggplot(agg_scree, aes(x = Component, y = Value, color = Strategy)) +
    ...
    geom_vline(xintercept = mean_k_plot, linetype = "longdash", color = "black", linewidth = 1) +
    annotate("text", x = mean_k_plot + 0.5, y = y_anno, 
             label = sprintf("Avg Haystack Elbow (k = %.2f)", mean_k_plot), 
             color="black", vjust = 0, hjust = 1, size = 4) +
    scale_y_continuous(trans="log10") +
    coord_cartesian(xlim = c(1, max(10, mean_k_plot * 2))) + 
    scale_color_manual(values = strat_colors, labels = labels_map) +
    labs(title = paste("Spectral Analysis by Feature Selection Strategy"),
         subtitle = sprintf("Generative Mode: %s -- Avg over %d iter(s). Dynamic signal evaluated at iteration-specific Haystack Elbows.", S2_GENERATION_MODE, n_iters),
...
run_one_full_simulation_iteration <- function(iteration_seed, m_schedule, k_eff_latent, inner_threads = 1, save_artifacts = FALSE) {
  ...
  IDEALIZED_DEPTH_COUNT <- min(2 * k_eff_idealized, 2 * M_PREDICTORS)
    ...
    for(error_level in scenarios_to_run) {
      ...
      for(strat in strategies) {
        ...
        res_list <- list(
          ...
          actual_depth_vars = IDEALIZED_DEPTH_COUNT
        )
...
plot_auroc_results <- function(agg_results, current_iter, total_iters, sparsity_results) {
  ...
  depth_label_text <- sprintf("Idealized Depth (%.2f vars) = %.3f", agg_results$mean_actual_depth_vars[1], agg_results$mean_depth_auroc[1])
...
plot_auprc_results <- function(agg_results, current_iter, total_iters, sparsity_results) {
  ...
  depth_label_text <- sprintf("Idealized Depth (%.2f vars) = %.3f", agg_results$mean_actual_depth_vars[1], agg_results$mean_depth_auprc[1])
...
plot_cond_entropy_results <- function(agg_results, current_iter, total_iters, sparsity_results) {
  ...
  depth_label_text <- sprintf("Idealized Depth (%.2f vars) = %.3f", agg_results$mean_actual_depth_vars[1], agg_results$mean_depth_cond_entropy[1])
...
run_full_simulation <- function(n_super_iterations = N_SUPER_ITERATIONS) {
  ...
  repeat {
    ...
    cat("\n--- S(1) Latent Complexity Simulation Results ---\n")
    ...
    cat(sprintf("  * K_eff_S'(2)_Obs_Est (from %d-run avg) = %.1f\n", N_SPECTRAL_ITERATIONS, k_eff_obs_est))
  ...
  repeat {
    ...
    for (i in start_iter:end_iter) {
      ...
      agg_results <- all_results_dt[, .(
        ...
        mean_actual_depth_vars = mean(actual_depth_vars)
  ...
  repeat {
    ...
    if (post_choice > 0 && post_choice < option_counter) {
      ...
      if (chosen_action == "AUROC") {
        ...
        cat("\nGenerating Spectral Comparison using dynamic haystack elbows...\n")
        run_strategy_spectral_comparison()
      }
